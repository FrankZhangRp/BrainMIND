...
def clean_reports(args, mode='direct', max_workers=5):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for report in reports:
            ...
            if mode == 'direct':
                findings = report['original_findings']
            elif mode == 'struct':
                findings = report.get('structured_findings', [])

            if args.add_clinical_info:
                clinical_info = report['clinical_info_summary']
                if args.language == 'chinese':
        ...
        for future, report in tqdm(futures, desc="Processing medical reports", total=len(futures)):
            try:
                ...
                if result:
                    ...
                    findings_input = []
                    if mode == 'direct':
                        findings_input = report['original_findings']
                    elif mode == 'struct':
                        findings_input = report.get('structured_findings', [])
                    ...
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump({
                            ...
                            'age': report['age'],
                            'input_findings': findings_input,
                            'reasoning_process': cleaned_reasoning,
                            'inferred_conclusion': cleaned_conclusion,
                            'label_conclusion': report['standardized_conclusion'],
